...
export class HybridStorageService implements DatabaseInterface, LegacyClientOperations {
  ...
  async createUser(user: Omit<DatabaseUser, 'id' | 'created_at' | 'updated_at'>): Promise<StorageResult<DatabaseUser>> {
    ...
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📴 Offline - user created locally only'); }
    }
  ...
  async updateUser(id: string, updates: Partial<DatabaseUser>): Promise<StorageResult<DatabaseUser>> {
    ...
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📴 Offline - user updated locally only'); }
    }
    const cached = await this.getFromAsyncStorage<DatabaseUser>(HYBRID_KEYS.USER_CACHE);
    const index = cached.findIndex(u => u.id === id);
    if (index !== -1) {
  ...
  async deleteUser(id: string): Promise<StorageResult<void>> {
    ...
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📴 Offline - user deleted locally only'); }
    }
    const cached = await this.getFromAsyncStorage<DatabaseUser>(HYBRID_KEYS.USER_CACHE);
    const filtered = cached.filter(u => u.id !== id);
    await this.setToAsyncStorage(HYBRID_KEYS.USER_CACHE, filtered);
  ...
  async addClient(name: string, hourlyRate: number, email?: string): Promise<Client> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📊 HybridStorage: addClient called with name:', name, 'rate:', hourlyRate, 'email:', email); }
    }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      ...
      if (user.success && user.data) {
        if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Client created in Supabase with ID:', user.data.id); }
      }
    } else {
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - client created locally only'); }
      }
    }
    ...
    if (__DEV__) {
      ...
      if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Created client with unique ID:', newClient.id, 'for', newClient.name); }
  ...
  async getClients(): Promise<Client[]> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📊 HybridStorage: getClients called'); }
    }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      ...
      if (users.success && users.data) {
        ...
        await this.setToAsyncStorage(LEGACY_KEYS.CLIENTS, clients);
        if (__DEV__) {
          if (__DEV__) { if (__DEV__) console.log('📊 HybridStorage: Loaded clients from Supabase:', clients.length, 'clients'); }
        }
        return clients;
      }
    }
    ...
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('📊 HybridStorage: Loaded clients from AsyncStorage:', sortedClients.length, 'clients'); }
    }
  ...
  async getClientById(id: string): Promise<Client | null> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🔍 HybridStorage: getClientById called with ID:', id); }
    }
    const clients = await this.getClients();
    if (__DEV__) { if (__DEV__) console.log('📋 HybridStorage: Available clients:', clients.map(c => ({ id: c.id, name: c.name }))); }
    const foundClient = clients.find(client => client.id === id) || null;
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🎯 HybridStorage: Found client:', foundClient ? foundClient.name : 'NOT FOUND'); }
    }
    return foundClient;
  }
  ...
  async updateClient(id: string, name: string, hourlyRate: number, email?: string): Promise<void> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('✏️ HybridStorage: updateClient called with ID:', id, 'name:', name, 'rate:', hourlyRate, 'email:', email); }
    }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - client updated locally only'); }
      }
    }
    ...
    await this.setToAsyncStorage(LEGACY_KEYS.CLIENTS, clients);
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Client updated successfully'); }
    }
  }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (error) {
          ...
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('📱 Session saved locally only - will need manual sync when online'); }
          }
        } else {
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('✅ Session saved to Supabase:', newSession.id); }
          }
        }
      } catch (error) {
        console.error('❌ Session save error:', error);
        if (__DEV__) {
          if (__DEV__) { if (__DEV__) console.log('📱 Session saved locally only'); }
        }
      }
    } else {
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - session saved locally only'); }
      }
    }
  ...
  async endSession(sessionId: string): Promise<Session> {
    ...
    if (__DEV__) { if (__DEV__) console.log(`💰 HybridStorage: Ending session for ${client.name}: ${duration.toFixed(2)}h × $${currentRate}/h = $${amount.toFixed(2)}`); }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (error) {
          ...
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('📱 Session updated locally only - will need manual sync when online'); }
          }
        } else {
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('✅ Session updated in Supabase:', sessions[sessionIndex].id); }
          }
        }
      } catch (error) {
        console.error('❌ Session update error:', error);
        if (__DEV__) {
          if (__DEV__) { if (__DEV__) console.log('📱 Session updated locally only'); }
        }
      }
    } else {
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - session updated locally only'); }
      }
    }
  ...
  async requestPayment(clientId: string, sessionIds: string[]): Promise<void> {
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (__DEV__) {
          if (__DEV__) { if (__DEV__) console.log('✅ Session statuses updated to requested in Supabase'); }
        }
      } catch (error) {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - session statuses updated to requested locally only'); }
      }
    }
  ...
  async markPaid(
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (paymentError) {
          ...
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('✅ Payment saved to Supabase:', payment.id); }
          }
        }
      } catch (error) {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - payment saved locally only'); }
      }
    }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (__DEV__) {
          if (__DEV__) { if (__DEV__) console.log('✅ Session statuses updated in Supabase'); }
        }
      } catch (error) {
        console.error('❌ Session status update error:', error);
      }
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - session statuses updated locally only'); }
      }
    }
    ...
    if (isOnline && this.config.primary === 'supabase') {
      try {
        ...
        if (error) {
          ...
          if (__DEV__) {
            if (__DEV__) { if (__DEV__) console.log('✅ Activity saved to Supabase:', newActivity.id); }
          }
        }
      } catch (error) {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('📴 Offline - activity saved locally only'); }
      }
    }
  }
  ...
  async initializeWithSeedData(): Promise<void> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🌱 HybridStorage: Initializing seed data...'); }
    }
    const clients = await this.getClients();
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🌱 HybridStorage: Current clients count:', clients.length); }
    }

    if (clients.length > 0) {
      if (__DEV__) { if (__DEV__) console.log('🌱 HybridStorage: Seed data already exists, skipping initialization'); }
      return;
    }

    if (__DEV__) {

      if (__DEV__) { if (__DEV__) console.log('🌱 HybridStorage: No existing data, creating seed data...'); }
    ...
    if (__DEV__) {
      ...
      if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Seed data initialization completed successfully!'); }

    }
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Created clients:', [client1.name, client2.name, client3.name, client4.name]); }
    }
  }

  async clearAllDataAndReinitialize(): Promise<void> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🧹 HybridStorage: Clearing all data and reinitializing...'); }
    }
    ...
    if (__DEV__) {
      ...
      if (__DEV__) { if (__DEV__) console.log('🧹 HybridStorage: All data cleared, reinitializing...'); }

    }
    await this.initializeWithSeedData();
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('✅ HybridStorage: Data cleared and reinitialized successfully!'); }
    }
  }
  ...
  private async getCurrentProviderId(): Promise<string> {
    try {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.warn('⚠️ Could not determine current provider ID, using fallback'); }
      }
  ...
  async clearSyncQueue(): Promise<void> {
    if (__DEV__) {
      if (__DEV__) { if (__DEV__) console.log('🧹 HybridStorage: Comprehensive sync queue cleanup starting...'); }
    }
    ...
    try {
      ...
      if (__DEV__) {
        if (__DEV__) { if (__DEV__) console.log('✅ Inspected and cleaned invalid operations'); }
      }
      ...
      if (__DEV__) {
        if (__DEV__) {
          if (__DEV__) console.log('✅ Cleared main SyncQueue');
        }
      }
      ...
      if (__DEV__) {
        if (__DEV__) {
          if (__DEV__) console.log('✅ Cleared offline queue');
        }
      }
      ...
      for (const key of allSyncKeys) {
        try {
          ...
          if (__DEV__) {
            if (__DEV__) {
              if (__DEV__) console.log(`✅ Cleared storage key: ${key}`);
            }
          }
        } catch (error) {
          if (__DEV__) {
            if (__DEV__) {
              if (__DEV__) console.warn(`⚠️ Could not clear key ${key}:`, error);
            }
          }
        }
      }
      ...
      if (__DEV__) {
        ...
        if (__DEV__) {
          if (__DEV__) console.log('✅ HybridStorage: Comprehensive sync queue cleanup completed');
        }
